...
result_book = op.load_workbook('/Users/snp/web/Selenium/middleschool.xlsx') ## output file directory
...
result_sheet.cell(row=1, column=2).value = largearea_category.text
largearea_category.click()
#### 서울특별시

### 3. [지역구] 클릭
smallarea_category = driver.find_element(By.XPATH,'//*[@id="level3Box"]/li[1]/label')
result_sheet.cell(row=1, column=3).value = smallarea_category.text
...
result_sheet.cell(row=1, column=4).value = middleschool_name.text
...
result_sheet.cell(row=1, column=5).value = num
result_book.save('/Users/snp/web/Selenium/middleschool.xlsx')
